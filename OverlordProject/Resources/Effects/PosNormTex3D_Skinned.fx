...
VS_OUTPUT VS(VS_INPUT input){
	...
	for (int i=0; i<4; i++)
	{
		...
		{
			transformedPosition += mul(originalPosition, gBones[currentBoneIdx]) * input.blendWeights[i];
			transformedNormal += mul(input.normal, (float3x3)gBones[currentBoneIdx]) * input.blendWeights[i];
		}
	}
