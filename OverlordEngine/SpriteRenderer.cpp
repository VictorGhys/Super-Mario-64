...
void SpriteRenderer::UpdateBuffer(const GameContext& gameContext)
{
	...
	if (m_pVertexBuffer == nullptr || m_Sprites.size() > m_BufferSize)
	{
		...
		bd.ByteWidth = m_InputLayoutSize * (uint32_t)m_Sprites.size();
		...
		result = gameContext.pDevice->CreateBuffer(&bd, &initData, &m_pVertexBuffer);
		if (FAILED(result))
			return;
	}
	//------------------------
	...
	memcpy(subresource.pData, m_Sprites.data(), m_InputLayoutSize * (uint32_t)m_Sprites.size());
